...
## Current status
- **Phase:** **Slice 3 (tool wrapper) COMPLETE (T3.1–T3.5).** `python -m leaklens scan --config config.yaml` runs end-to-end → scorecard + JSON/HTML reports. Slice 2 pipeline COMPLETE (T2.1–T2.4); still awaiting the real `--full`/num_steps=20 run for the citable numbers + the written finding (T2.5).
- **Last done:** T3.5 — `leaklens/runner.py` (isolating runner) + `cli.py` + `__main__.py` + `[project.scripts]` + `tests/test_runner.py` (4) + `tests/test_cli.py` (3, e2e). Full suite 86 passed / 1 skipped (slow).
- **Next action:** Slice 4 — validation harness (the moat): `scripts/build_validation_targets.py` (≥3 leaky + ≥3 hardened) + `validation/harness.py` + `test_validation.py` + `results/validation_matrix.md`. In parallel track: run the real `--full --num-steps 20` sweep (Colab), re-render the plot, write T2.5, re-confirm the 0.6 leak threshold (D9).
