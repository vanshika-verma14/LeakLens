...
## Current status
- **Phase:** Slice 3 (tool wrapper) — T3.1 + T3.2 + T3.3 done. Slice 2 pipeline COMPLETE (T2.1–T2.4); still awaiting the real `--full`/num_steps=20 run for the citable numbers + the written finding (T2.5).
- **Last done:** T3.3 — `leaklens/modules/base.py` (`Module` contract) + `leaklens/modules/inversion.py` (`InversionModule`: wraps the core → `Finding`; honest-failure gate before any model load; verdict from the config threshold; self-isolating) + `tests/test_module_inversion.py` (5, fast fake inverter/adapter). Full suite 70 passed / 1 skipped (slow).
- **Next action:** T3.4 `report/owasp.py` + `report/scorecard.py` + `report/json_report.py` + `report/html_report.py`. In parallel track: run the real `--full --num-steps 20` sweep (Colab), re-render the plot, write T2.5, re-confirm the 0.6 leak threshold (D9).
